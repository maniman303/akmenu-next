...
eRunNdsRetCode runNds(const void* loader, u32 loaderSize, u32 cluster, bool initDisc, bool dldiPatchNds, int argc, const char** argv)
{
	...
	writeAddr ((data_t*) VRAM_C, STORED_FILE_CLUSTER_OFFSET, cluster);

	// INIT_DISC = initDisc;
	writeAddr ((data_t*) VRAM_C, INIT_DISC_OFFSET, initDisc);

	writeAddr ((data_t*) VRAM_C, DSIMODE_OFFSET, isDSiMode());
	if(argv[0][0]=='s' && argv[0][1]=='d') {
		dldiPatchNds = false;
		writeAddr ((data_t*) VRAM_C, HAVE_DSISD_OFFSET, 1);
	}

	// WANT_TO_PATCH_DLDI = dldiPatchNds;
	writeAddr ((data_t*) VRAM_C, WANT_TO_PATCH_DLDI_OFFSET, dldiPatchNds);
	// Give arguments to loader
	argStart = (char*)VRAM_C + readAddr((data_t*)VRAM_C, ARG_START_OFFSET);
	...
	*argData = argTempVal;
	...
	writeAddr ((data_t*) VRAM_C, ARG_START_OFFSET, (addr_t)argStart - (addr_t)VRAM_C);
	writeAddr ((data_t*) VRAM_C, ARG_SIZE_OFFSET, argSize);

	if(dldiPatchNds) {
		// Patch the loader with a DLDI for the card
		if (!dldiPatchLoader ((data_t*)VRAM_C, loaderSize, initDisc)) {
...
eRunNdsRetCode runNdsFile (const char* filename, int argc, const char** argv)  {
	...
	fprintf(stderr, "File cluster: %ld.", st.st_ino);
	flushConsole();
